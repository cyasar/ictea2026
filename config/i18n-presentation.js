/** Presentation mode translations — EN / TR / RO (27 slides: 24 main + 3 concept-reading appendix) */
window.PRES_I18N = {
  en: {
    ui: {
      exit: "Exit",
      ...
      conceptReading: "Optional · Concept reading",
    ...
    slides: [
      ...
      {
        appendix: true,
        title: "Concept Reading · QUBO & Binary Variables",
        math: [
          "QUBO = Quadratic Unconstrained Binary Optimization",
          "Design vector:  x = (x₁, x₂, …, xₙ),   xᵢ ∈ {0, 1}",
          "Search space:  |Ω| = 2ⁿ binary configurations",
          "Goal:  find x*  such that  H(x*) = min H(x)",
        ],
        body: [
          "xᵢ = 1 → pore i is active (O₂ pathway open)",
          "xᵢ = 0 → pore i is passive",
          "QUBO encodes rewards and penalties in one scalar H(x)",
          "Not a quantum-superiority claim — a modeling language",
        ],
        note: "Optional concept slide for the audience. QUBO reformulates pore selection as binary optimization. Each of n pores is a yes/no decision. With n=6 there are 64 patterns; with n=48 there are 2⁴⁸ — hence different solvers for small vs large networks.",
      },
      {
        appendix: true,
        title: "Concept Reading · Hamiltonian H(x)",
        math: [
          "H(x) = − Σᵢ fᵢ xᵢ  +  Σᵢ<ⱼ qᵢⱼ xᵢ xⱼ",
          "       + λₚ [max(0, Σᵢ pᵢ xᵢ − Pmax)]²",
          "       + λₖ [max(0, Σᵢ xᵢ − K)]²  +  λc C(x)",
        ],
        body: [
          "−Σ fᵢ xᵢ — reward activating efficient pores (negative = good)",
          "Σ qᵢⱼ xᵢ xⱼ — hydraulic coupling between active pairs",
          "λₚ term — penalizes total pressure above Pmax",
          "λₖ term — penalizes more than K active pores",
          "C(x) — penalizes disconnected channel paths",
        ],
        note: "Term-by-term reading of the Hamiltonian. Minimizing H(x) balances flow efficiency against constraint violations. Squared penalties are soft constraints: they vanish at the feasible optimum (e.g. P exactly at Pmax, exactly K pores active).",
      },
      {
        appendix: true,
        title: "Concept Reading · Ising, Metrics & Solvers",
        math: [
          "Ising map:  xᵢ = (1 + sᵢ) / 2,   sᵢ ∈ {−1, +1}",
          "Efficiency:  η(x) = Σᵢ fᵢ xᵢ",
          "Pressure:    P(x) = Σᵢ pᵢ xᵢ",
          "Feasible iff  P(x) ≤ Pmax  and  Σᵢ xᵢ ≤ K",
        ],
        body: [
          "Ising spins sᵢ link binary pores to spin hardware / annealers",
          "Exhaustive search — exact for small n (e.g. 2⁶ = 64 states)",
          "Simulated annealing — heuristic for large n (e.g. 2⁴⁸)",
          "Same H(x) landscape for classical, heuristic, and Ising-inspired solvers",
        ],
        note: "Optional closing glossary. η and P are observables computed from x. Feasibility is checked independently of H(x). Exhaustive search validates the six-pore benchmark; simulated annealing scales to the 6×8 network. No quantum hardware advantage is claimed in this PoC.",
      },
    ],
  },
  tr: {
    ui: {
      ...
      conceptReading: "İsteğe bağlı · Kavram okuması",
    ...
    slides: [
      ...
      {
        appendix: true,
        title: "Kavram Okuması · QUBO ve İkili Değişkenler",
        math: [
          "QUBO = Quadratic Unconstrained Binary Optimization",
          "Tasarım vektörü:  x = (x₁, x₂, …, xₙ),   xᵢ ∈ {0, 1}",
          "Arama uzayı:  |Ω| = 2ⁿ ikili konfigürasyon",
          "Amaç:  x* bul  öyle ki  H(x*) = min H(x)",
        ],
        body: [
          "xᵢ = 1 → i. gözenek aktif (O₂ yolu açık)",
          "xᵢ = 0 → i. gözenek pasif",
          "QUBO ödül ve cezaları tek skaler H(x) içinde kodlar",
          "Kuantum üstünlüğü iddiası değil — modelleme dili",
        ],
        note: "İsteğe bağlı kavram slaytı. QUBO, gözenek seçimini ikili optimizasyona dönüştürür. n=6 için 64, n=48 için 2⁴⁸ desen vardır; bu yüzden küçük ve büyük ağlarda farklı çözücüler kullanılır.",
      },
      {
        appendix: true,
        title: "Kavram Okuması · Hamiltonian H(x)",
        math: [
          "H(x) = − Σᵢ fᵢ xᵢ  +  Σᵢ<ⱼ qᵢⱼ xᵢ xⱼ",
          "       + λₚ [max(0, Σᵢ pᵢ xᵢ − Pmax)]²",
          "       + λₖ [max(0, Σᵢ xᵢ − K)]²  +  λc C(x)",
        ...
        body: [
          "−Σ fᵢ xᵢ — verimli gözenekleri aktive etmeyi ödüllendirir",
          "Σ qᵢⱼ xᵢ xⱼ — aktif çiftler arası hidrolik etkileşim",
          "λₚ terimi — toplam basıncı Pmax üstünde cezalandırır",
          "λₖ terimi — K'den fazla aktif gözenek cezası",
          "C(x) — kopuk kanal yollarını cezalandırır",
        ],
        note: "Hamiltonian terim terim okunur. H(x) minimizasyonu verimlilik ile kısıt ihlallerini dengeler. Kare cezalar yumuşak kısıttır; uygulanabilir optimumda sıfıra iner (ör. P tam Pmax'ta, tam K gözenek aktif).",
      },
      {
        appendix: true,
        title: "Kavram Okuması · Ising, Metrikler ve Çözücüler",
        math: [
          "Ising dönüşümü:  xᵢ = (1 + sᵢ) / 2,   sᵢ ∈ {−1, +1}",
          "Verimlilik:  η(x) = Σᵢ fᵢ xᵢ",
          "Basınç:      P(x) = Σᵢ pᵢ xᵢ",
          "Uygulanabilir  ⟺  P(x) ≤ Pmax  ve  Σᵢ xᵢ ≤ K",
        ],
        body: [
          "Ising spinleri sᵢ ikili gözenekleri spin donanımına bağlar",
          "Exhaustive search — küçük n için kesin (2⁶ = 64 durum)",
          "Simulated annealing — büyük n için sezgisel (2⁴⁸)",
          "Aynı H(x) manzarası: klasik, sezgisel ve Ising-esinli çözücüler",
        ],
        note: "İsteğe bağlı kapanış sözlüğü. η ve P, x'ten hesaplanan gözlemlenebilirler. Uygulanabilirlik H(x)'ten bağımsız kontrol edilir. Altı gözenek exhaustive ile doğrulanır; 6×8 ağ SA ile ölçeklenir.",
      },
    ],
  },
  ro: {
    ui: {
      ...
      conceptReading: "Opțional · Citire concepte",
    ...
    slides: [
      ...
      {
        appendix: true,
        title: "Citire concepte · QUBO & variabile binare",
        math: [
          "QUBO = Quadratic Unconstrained Binary Optimization",
          "Vector design:  x = (x₁, x₂, …, xₙ),   xᵢ ∈ {0, 1}",
          "Spațiu căutare:  |Ω| = 2ⁿ configurații binare",
          "Scop:  găsește x*  astfel încât  H(x*) = min H(x)",
        ],
        body: [
          "xᵢ = 1 → porul i activ (cale O₂ deschisă)",
          "xᵢ = 0 → porul i inactiv",
          "QUBO codifică recompense și penalizări în H(x)",
          "Nu este revendicare cuantică — limbaj de modelare",
        ],
        note: "Slide opțional de concepte. QUBO reformulează selecția porilor ca optimizare binară. n=6 → 64 pattern-uri; n=48 → 2⁴⁸ — de aceea solvere diferite pentru rețele mici vs mari.",
      },
      {
        appendix: true,
        title: "Citire concepte · Hamiltonian H(x)",
        math: [
          "H(x) = − Σᵢ fᵢ xᵢ  +  Σᵢ<ⱼ qᵢⱼ xᵢ xⱼ",
          "       + λₚ [max(0, Σᵢ pᵢ xᵢ − Pmax)]²",
          "       + λₖ [max(0, Σᵢ xᵢ − K)]²  +  λc C(x)",
        ...
        body: [
          "−Σ fᵢ xᵢ — recompensă pori eficienți (negativ = bine)",
          "Σ qᵢⱼ xᵢ xⱼ — cuplaj hidraulic între perechi active",
          "λₚ — penalizează presiunea totală peste Pmax",
          "λₖ — penalizează mai mult de K pori activi",
          "C(x) — penalizează căi de canal deconectate",
        ],
        note: "Citire termen cu termen a Hamiltonianului. Minimizarea H(x) echilibrează eficiența cu încălcările constrângerilor. Penalizările pătrat dispar la optimul fezabil.",
      },
      {
        appendix: true,
        title: "Citire concepte · Ising, metrici & solvere",
        math: [
          "Mapare Ising:  xᵢ = (1 + sᵢ) / 2,   sᵢ ∈ {−1, +1}",
          "Eficiență:  η(x) = Σᵢ fᵢ xᵢ",
          "Presiune:    P(x) = Σᵢ pᵢ xᵢ",
          "Fezabil  ⟺  P(x) ≤ Pmax  și  Σᵢ xᵢ ≤ K",
        ],
        body: [
          "Spini Ising sᵢ leagă porii binari de hardware spin/annealer",
          "Căutare exhaustivă — exactă pentru n mic (2⁶ = 64)",
          "Simulated annealing — euristică pentru n mare (2⁴⁸)",
          "Același peisaj H(x) pentru solvere clasice și Ising",
        ],
        note: "Glosar opțional de închidere. η și P sunt observabile din x. Fezabilitatea se verifică independent de H(x). Benchmark șase pori cu exhaustive; rețea 6×8 cu SA.",
      },
    ],
  },
};
